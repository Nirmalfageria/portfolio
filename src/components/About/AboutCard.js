import React from "react";
import Card from "react-bootstrap/Card";
import { ImPointRight } from "react-icons/im";

function AboutCard() {
  return (
    <Card className="quote-card-view">
      <Card.Body>
        <blockquote className="blockquote mb-0">
          <p style={{ textAlign: "justify" }}>
            Hi Everyone, I am <span className="purple">Nirmal Fageria </span>
            from <span className="purple"> Rajasthan, India.</span>
            <br />
            {/* I am currently employed as a software developer at Juspay.
            <br /> */}
            I currently pursuing my B.Tech in Electrical and Electronics Engineering from NIT Calicut.
            <br />
            <br />
            Apart from coding, some other activities that I love to do!
          </p>
          <ul>
            <li className="about-activity">
              <ImPointRight /> Playing Games
            </li>
            <li className="about-activity">
              <ImPointRight /> Reading Books and Blogs
            </li>
            <li className="about-activity">
              <ImPointRight /> Travelling
            </li>
          </ul>

          <p style={{ color: "rgb(155 126 172)" }}>
            "I believe in the power of innovation to drive progress"
          </p>
          <footer className="blockquote-footer">Ratan Tata</footer>
        </blockquote>
      </Card.Body>
    </Card>
  );
}

export default AboutCard;
